...
// `auto_inline_extensions` list (stored in config.json): a file previews as text
// iff its extension is in that list. The list is seeded with sensible text
...
export function getPreviewType(
  ...
  if (autoInlineExts.includes(ext)) return "text";
  return null;
}
