...
- **Prometheus metrics** at `/metrics` (optional basic auth via `METRICS_PASSWORD`). Covers HTTP, auth, S3 ops, MCP tool calls, DB query duration. Metric inventory + scrape config: `docs/observability.md`.
